...
Launched as a subprocess by :class:`mlx_server.backend.ModelBackend`.

Usage::

    python embedding_server.py <model_id> <port>
"""
...
class EmbeddingHandler(http.server.BaseHTTPRequestHandler):
    """Handles /v1/embeddings and /v1/models for a single embedding model."""
    ...
    def do_POST(self):
        ...
        texts: list[str] = (
            [raw_input] if isinstance(raw_input, str) else raw_input
        )
        ...
        try:
            ...
            log.debug("Encoded %d text(s) in %.2fs", len(texts), time.time() - t0)
        except Exception as exc:
            log.exception("Encode error")
            self._json(500, self._error(str(exc)))
            return

        # Approximate token count via tokenizer; fall back to word count
        try:
            encoded = self.model.tokenize(texts)
            token_count = int(
                encoded["input_ids"].shape[0] * encoded["input_ids"].shape[1]
            )
        except Exception:
...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: embedding_server.py <model_id> <port>",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    EmbeddingHandler.model_id = model_id
